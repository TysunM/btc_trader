"""Apply all configured feature generators to the merged data table."""
...
def main(config_file: str) -> None:
    ...
    require_fields(config, ["symbol", "data_folder", "time_column", "feature_sets"])
    ...
    file_path = data_path / config["merge_file_name"]
    ...
    if not feature_sets:
        print("ERROR: no feature_sets defined. Nothing to process.")
        return
    ...
    for i, fs in enumerate(feature_sets):
        fs_now = datetime.now()
        print(f"Feature set {i}/{len(feature_sets)}: {fs.get('generator')}...")
        df, new_features = generate_feature_set(df, fs, config, model_store, last_rows=0)
        all_features.extend(new_features)
        print(f"  -> {len(new_features)} features. {str(datetime.now() - fs_now).split('.')[0]}")

    df = df.replace([np.inf, -np.inf], np.nan)
    na_count = df[all_features].isna().any(axis=1).sum()
    if na_count:
        print(f"WARNING: {na_count} rows have NaN in some feature column(s) (expected at the start of the series).")

    out_path = data_path / config["feature_file_name"]
    print(f"Storing {len(df)} records / {len(df.columns)} columns to {out_path}...")
    write_data_file(df, out_path)

    with open(out_path.with_suffix(".txt"), "a+") as f:
        f.write(", ".join(f'"{x}"' for x in all_features) + "\n\n")

    print(f"Finished generating {len(all_features)} features in {str(datetime.now() - now).split('.')[0]}")
